...
let header (ctx : SiteContents) page =
    if ctx.TryGetValue<Globalloader.SiteInfo>().IsSome then
    ...
    else failwithf "Site context container no Globalloader.SiteInfo:%s %A" (System.Environment.NewLine) (ctx.Errors())
